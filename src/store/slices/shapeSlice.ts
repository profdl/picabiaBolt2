...
export const shapeSlice: StateCreator<ShapeSlice, [], [], ShapeSlice> = (set, get) => ({
    shapes: [],
    selectedShapes: [],
    clipboard: [],
    history: [[]],
    ...
    addShapes: (newShapes) => set(state => {
        ...
        return {
            ...
            history: [...state.history.slice(0, state.historyIndex + 1), updatedShapes].slice(-MAX_HISTORY),
            historyIndex: state.historyIndex + 1,
            selectedShapes: newShapes.map(shape => shape.id)
        };
    }),

    updateShape: (id: string, props: Partial<Shape>) => set(state => {
        const newShapes = state.shapes.map(shape => {
            if (shape.id === id) {
                return {
                    ...
                    showScribble: 'showScribble' in props ? props.showScribble : shape.showScribble,
                    showRemix: 'showRemix' in props ? props.showRemix : shape.showRemix,
                    showPrompt: 'showPrompt' in props ? props.showPrompt : shape.showPrompt
                };
            }
            return shape;
        });
    ...
    updateShapes: (updates: { id: string; shape: Partial<Shape> }[]) => set(state => {
        const newShapes = state.shapes.map(shape => {
            ...
            if (update) {
                return {
                    ...shape,
                    ...update.shape,
                    showDepth: 'showDepth' in update.shape ? update.shape.showDepth : shape.showDepth,
                    showEdges: 'showEdges' in update.shape ? update.shape.showEdges : shape.showEdges,
                    showPose: 'showPose' in update.shape ? update.shape.showPose : shape.showPose,
                    showScribble: 'showScribble' in update.shape ? update.shape.showScribble : shape.showScribble,
                    showRemix: 'showRemix' in update.shape ? update.shape.showRemix : shape.showRemix,
                    showPrompt: 'showPrompt' in update.shape ? update.shape.showPrompt : shape.showPrompt
                };
            }
            return shape;
        });
    ...
    sendToBack: () => set(state => {
        const selectedShapeObjects = state.shapes.filter(s => state.selectedShapes.includes(s.id));
        const otherShapes = state.shapes.filter(s => !state.selectedShapes.includes(s.id));
    ...
    createGroup: (shapeIds) => set(state => {
