...
export function useCanvasMouseHandlers() {
    ...
    const getCanvasPoint = (e: React.MouseEvent | React.DragEvent, canvasRef: React.RefObject<HTMLDivElement>): Position => {
    ...
    const handleMouseDown = (e: React.MouseEvent, canvasRef: React.RefObject<HTMLDivElement>, spacePressed: boolean) => {
    ...
    const handleMouseMove = (e: React.MouseEvent, canvasRef: React.RefObject<HTMLDivElement>) => {
        ...
        const isEditingSticky = shapes.some(shape =>
            shape.type === 'sticky' && selectedShapes.includes(shape.id) && shape.isEditing
        );
