            ...
            {showAdvanced && (
              <div className="space-y-2">
                <div>
                  ...
                  <SmallSlider
                    value={shape.steps || 45}
                    onChange={(value) => updateShape(shape.id, { steps: value })}
                    min={1}
                    max={100}
                    step={1}
                    label="Steps"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={shape.seed?.toString() || Math.floor(Math.random() * 1000000).toString()}
                    onChange={(e) =>
                      updateShape(shape.id, { seed: Number(e.target.value) })
                    }
                    disabled={shape.randomiseSeeds}
                    className={`w-full py-1 px-2 text-xs border rounded bg-white dark:bg-neutral-700 border-neutral-200 dark:border-neutral-600 text-neutral-900 dark:text-neutral-100 block ${
                      shape.randomiseSeeds ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  />
                </div>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const newSeed = isRandom ? Math.floor(Math.random() * 1000000) : (shape.seed || Math.floor(Math.random() * 1000000));
                      updateShape(shape.id, {
                        randomiseSeeds: isRandom,
                        seed: newSeed
                      });
                    }}
