...
export const Toolbar: React.FC<ToolbarProps> = ({ showGallery }) => {
  ...
  const handleCrop = (e: React.MouseEvent) => {
    ...
    if (selectedShape && selectedShape.type === "image") {
      // Create a root div element for the ImageCropper modal
      const rootDiv = document.createElement('div');
      rootDiv.id = 'crop-modal-root';
      document.body.appendChild(rootDiv);
      
      // Create a root using createRoot
      const root = createRoot(rootDiv);
      
      // Render the ImageCropper component
      const imageUrl = (selectedShape as ImageShape).imageUrl || '';
      const cleanup = () => {
